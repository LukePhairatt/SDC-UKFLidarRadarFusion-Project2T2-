...
int main()
{
  ...
  // Create a PID instance for a distance and turning control 
  // Set the gains to suit the system 
  PID pid_dist(0.5,0.0,0.1,false);   // Kp,Ki,Kd,no normalised angle
  PID pid_turn(0.9,0.005,0.0,true);  // Kp,Ki,Kd,need normalised angle
  
  
  // My note: lambda function [references to be used inside]
  h.onMessage([&ukf,&target_x,&target_y, &pid_dist, &pid_turn](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          //My note: send diff turn and distance to the simulator for the hunter path control
          //         the chaser will be moved to the target after this step by this time the target already moves to the next point
          //         so we one always one step behind we if don't predict the position ahead!
          //My note: make sure UKF give the right position (target_x, target_y)
          //         system instability (e.g. spining) when not moving very much say the chaser is near the target 
          //         this is to do with the sim bug to handle the control point? (//TODO check the angle and distance tolerance)
          
          ///* Using PID loop to steer to the target (should work without the pid) 
          //distance_difference = pid_dist.RunPID(distance_difference); //TODO limit range and negative
          heading_difference = pid_turn.RunPID(heading_difference);
          ///*
  ...
  h.onConnection([&h](uWS::WebSocket<uWS::SERVER> ws, uWS::HttpRequest req) {
    std::cout << "Connected!!! Challenge" << std::endl;
  });
